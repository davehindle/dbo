 ...
 */
	function addFilter($f) {
		...
		foreach($f->constraints as $c) {
			if (!array_key_exists($c->sqlColumn(), $this->columns)) throw new exception('Column '.$c->column.' does not exist in table '.$this->schema.'.'.$this->name);
		}
 ...
 */
	...
	private function selectFromWhere($showKeys, &$select, &$from, &$where) {
		$select = '';
		$from = '';
		$where1 = '';
		$where2 = '';
		...
		do {
			...
			foreach ($table->filters as $f) {
				...
				foreach ($f->constraints as $c) {
					$subclause .= ($subclause == '' ? '' : ' AND ')."$t1.".$c->sqlColumn()." ".$c->sqlOperator()." ".$c->sqlOperand($table->store)."";
				}
